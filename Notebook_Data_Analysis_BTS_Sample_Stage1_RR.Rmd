...
df_PSA001 <- select(df_PSA001, user_id, age,sex,country_map)
#合并重复的行
df_PSA001 <- distinct(df_PSA001)

df_PSA001 <- df_PSA001 %>%
count(country_map)  %>% arrange(-n)
...
```{r}
wppdata3 <- wppdata2 %>% left_join(df_PSA001_country, by = "country_map")
#将台湾省的数据与中国内地的数据合并
...
pddata1 <- pddata[!is.na(pddata$ps2014),]
pddata2 <- pddata[!is.na(pddata$percentage_sample),]
pddata
```
