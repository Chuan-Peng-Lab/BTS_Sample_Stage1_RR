...
# maybe try binning for first_corresponding_author_ps2014
author$n_binned_fc_ps2014 <- 
  if_else(author$percentage_fc_author_ps2014 >= 0.16, "0.16-1",
 
  
  if_else(author$percentage_fc_author_ps2014 < 0.16 & author$percentage_fc_author_ps2014 >= 0.08, "0.08-0.16" , 
   if_else(author$percentage_fc_author_ps2014 < 0.04 & author$percentage_fc_author_ps2014 >= 0.08, "0.04-0.08" ,
   
  if_else(author$percentage_fc_author_ps2014 < 0.04 & author$percentage_fc_author_ps2014 >= 0.02, "0.02-0.04" ,
   if_else(author$percentage_fc_author_ps2014 < 0.02 & author$percentage_fc_author_ps2014 >= 0.01, "0.01-0.02" ,
    if_else(author$percentage_fc_author_ps2014 < 0.01 & author$percentage_fc_author_ps2014 >= 0.005, "0.005-0.01" ,        
  if_else(author$percentage_fc_author_ps2014 < 0.005 & author$percentage_fc_author_ps2014 >= 0.0025, "0.0025-0.005" ,
      "0-0.0025"
  )
 ))
))))
...
author_psa001 <- ggplot(author_bts_no_na) +
  ...
  scale_fill_manual(name = "Proportion of authors",
                    values = c("#CCE5FF","#99CCFF", "#66B2FF","#3399FF","#0080FF" ,"#0066CC","#004C99","#003366"),
                    limits = c("0-0.0025", "0.0025-0.005", "0.005-0.01", "0.01-0.02", "0.02-0.04", "0.04-0.08","0.08-0.16", "0.16-1")) +
ggtitle("Geographical distribution of authors from BTS")+
  theme(plot.title = element_text(hjust = 0.5, size = 15),
        legend.text = element_text(size = 10),  # 调整图例文字的大小
        legend.title = element_text(size = 12),  # 调整图例标题的大小
        legend.key.size = unit(3, "pt"),  # 调整图例键的尺寸
...
author_ps2014 <- ggplot(author_ps2022_no_na) +
  ...
  scale_fill_manual(name = "Proportion of authors",
                    values = c("#CCE5FF","#99CCFF", "#66B2FF","#3399FF","#0080FF" ,"#0066CC","#004C99","#003366"),
                    limits = c("0-0.0025", "0.0025-0.005", "0.005-0.01", "0.01-0.02", "0.02-0.04", "0.04-0.08","0.08-0.16", "0.16-1")) +
ggtitle("Geographical distribution of authors from traditional studies")+
  theme(plot.title = element_text(hjust = 0.5, size = 15),
        legend.text = element_text(size = 10),  # 调整图例文字的大小
        legend.title = element_text(size = 12),  # 调整图例标题的大小
        legend.key.size = unit(3, "pt"),  # 调整图例键的尺寸
...
```{r}
# 创建新的数据框 author_ps2022_no_na，剔除包含缺失值的行
author_ps2022_no_na <- author[complete.cases(author$n_binned_fc_ps2014), ]
author_ps2022_no_na

# map of PS2014 data 
author_ps2014_fc <- ggplot(author_ps2022_no_na) +
  geom_map(aes(map_id = country_map, fill = n_binned_fc_ps2014), map = world_map_new) +
  geom_polygon(data = world_map_new, 
               aes(x = long, y = lat, group = group), 
               colour = 'black', size=0.1, fill = NA) + 
  theme_void() + 
  ...
  scale_fill_manual(name = "Proportion of authors",
                    values = c("#CCE5FF","#99CCFF", "#66B2FF","#3399FF","#0080FF" ,"#0066CC","#004C99","#003366"),
                    limits = c("0-0.0025", "0.0025-0.005", "0.005-0.01", "0.01-0.02", "0.02-0.04", "0.04-0.08","0.08-0.16", "0.16-1")) +
ggtitle("Geographical distribution of first/corresponding_authors from traditional studies")+
  theme(plot.title = element_text(hjust = 0.5, size = 15),
        legend.text = element_text(size = 10),  # 调整图例文字的大小
        legend.title = element_text(size = 12),  # 调整图例标题的大小
        legend.key.size = unit(3, "pt"),  # 调整图例键的尺寸
...
```{r}  
author_ps2014_fc + country_people + plot_layout(ncol = 1, nrow = 3, heights = c(4, 4)) +
  plot_annotation(tag_levels = 'A')   # 调整图形标题的字体大小为12
...
author_bts_clean <- author_BTS$author_bts[!is.na(author_BTS$author_bts)]
author_ps2014_clean <- author_BTS$author_ps2014[!is.na(author_BTS$author_ps2014)]

first_corresponding_author_ps2014_clean <- author_BTS$first_corresponding_author_ps2014[!is.na(author_BTS$first_corresponding_author_ps2014)]

author_BTS
author_bts_clean
author_ps2014_clean
first_corresponding_author_ps2014_clean
```
...
α_ps2014_first_corresponding <- diversity(first_corresponding_author_ps2014_clean, index = "simpson")
# 输出结果
α_ps2014
α_bts
α_wpd
α_ps2014_first_corresponding
```
...
# 计算变量first_corresponding_author_ps2014的方差
N4 <- sum(first_corresponding_author_ps2014_clean)
sum_of_squares4 <- sum((first_corresponding_author_ps2014_clean / N4)^2)
sum_of_cubes4 <- sum((first_corresponding_author_ps2014_clean / N4)^3)

Var_ps2014_first_corresponding <- (4 * N4 * (N4 - 1) * (N4 - 2) * sum_of_cubes4 +
...
Var_bts
Var_wpd
Var_ps2014_first_corresponding
```
...
t_value3 <- (α_wpd-α_ps2014_first_corresponding) / sqrt(Var_wpd+Var_ps2014_first_corresponding)
  
df_3 <- (Var_wpd+Var_ps2014_first_corresponding)^2 / (Var_ps2014_first_corresponding^2 / N4 + Var_wpd^2 / N3)
# 计算 p 值
