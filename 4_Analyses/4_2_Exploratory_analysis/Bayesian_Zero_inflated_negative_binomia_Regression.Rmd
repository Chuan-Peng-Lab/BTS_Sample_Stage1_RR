...
x_zoom_range <- c(0, 5000)
y_zoom_range <- c(0, 0.001)

# 创建主数据，使用表达式格式的 group 标签
main_data <- tibble(x = c(400, 600),
                    y = c(0.00062, 0.00054),
                    meanlog = c(0, 6),
                    sdlog = c(4.67, 2.67)) %>% 
  ...
  mutate(
    ...
    group_label = glue("beta[0] %~% Normal({meanlog}, {sdlog})"),
    group = factor(group_label, levels = unique(group_label))
  )
...
main_plt <- main_data %>%
  ...
  scale_fill_manual(
    values = wesanderson::wes_palette("Moonrise2")[1:2],
    labels = parse_format()
  ) +
  scale_color_manual(
    values = wesanderson::wes_palette("Moonrise2")[1:2],
  ...
  xlab("Sample size of big team science")+ 
  annotate("rect",
           xmin = -10000, xmax =20000,
           ymin = -0.0003, ymax = 0.0013,
           color = "red", linetype = "solid", fill = NA, size = 0.5) +
  annotate("segment", x = 0, y = 0.0013, xend = 1190000 * 0.3, yend = 0.02 * 0.96,
           color = "grey80", linetype = "dashed", size = 1) +
  annotate("segment", x = 20000, y = 0, xend = 1190000 * 0.9, yend = 0.02 * 0.22,
           color = "grey80", linetype = "dashed", size = 1) +
  theme(
...
# 缩放图数据
zoom_data <- main_data %>% filter(number <= 5000)
...
zoom_plt <- zoom_data %>%
  ...
  scale_y_continuous(limits = y_zoom_range) +
  scale_x_continuous(limits = x_zoom_range) +
  theme_minimal(base_family = "Times") +
  theme(
    plot.background = element_rect(fill = NA, color = "grey80", size = 2),
    legend.position = "none",
    aspect.ratio = 1,
    panel.grid = element_blank(),
    panel.border = element_blank(),
...
# 组合图
combined_plot <- main_plt +
  inset_element(zoom_plt,
...
ggsave(filename = "pp1.pdf", plot = combined_plot, device = "pdf", width = 7, height = 4)
```
...
ggsave(filename = "pp2.pdf", plot = pp, device = "pdf", width = 8, height = 18)
```
...
ggsave(filename = "pp3.pdf", plot = pp1, device = "pdf", width = 6, height = 6)
```
...
main_plt<-  pp_check(gdp_zero_inflated_negbinomial_bts1,ndraws=2000)+
  theme_bw() +
  coord_cartesian()+
 xlim(0, 1190000) +
  ylim(-0.0003, 0.02) +
  ...
  annotate("rect",
           xmin = 0, xmax =20000,
           ymin = -0.0003, ymax = 0.0053,
           color = "red", linetype = "solid", fill = NA, size = 0.5) +
  annotate("segment", x = 0, y = 0.0053, xend = 1190000 * 0.275, yend = 0.02 * 0.99,
           color = "grey80", linetype = "dashed", size = 1) +
  annotate("segment", x = 20000, y = 0, xend = 1190000 * 0.999, yend = 0.02 * 0.31,
           color = "grey80", linetype = "dashed", size = 1) +
...
zoom_plt <- pp_check(gdp_zero_inflated_negbinomial_bts1,ndraws=200)+
  theme_bw() +
  xlim(0, 5000) +
  ylim(0, 0.0052) +
 coord_cartesian()+
...
combined_plot <- main_plt +
  inset_element(zoom_plt,
                left = 0.3, bottom = 0.35,
                right = 0.95, top = 0.95)

# 显示图
combined_plot

ggsave(filename = "pp4.pdf", plot = combined_plot, device = "pdf", width = 6, height = 4)
```

The visualization of the prior prediction check is shown below. We can see that the sample size can accommodate values up to 1,200,000. And, the actual data fall within the range of the prior distribution, demonstrating the appropriateness of the prior.
