---
title: "Exploratory_analysis_BTS_Sample_Stage1_RR"
author: "Weibiao Liu"
date: "2024/01/26"
output: html_document
---
This is an [R Markdown](http://rmarkdown.rstudio.com) file for Exploratory analysis of data analyses in the protocol of a meta research which aims at surveying sample characteristics of big team science in psychology. 

In this script, we will use data from PSA 001 project [Jones et al., 2021, *Nature Human Behavior*](https://doi.org/10.1038/s41562-020-01007-2), and partial articles published in psychological science in 2014 [Red et al., 2018, *Proceedings of the National Academy of Sciences*](https://doi.org/10.1073/pnas.1721165115), and Ruggeri et al. (2022)[*Nature Human Behavior*](https://doi.org/10.1038/s41562-022-01392-w) to exemplify the analyses that we are going to use in our this project.
...
## Number of universities per 100000 people
```{r}
...
university <- ggplot(combined_data1, aes(x = number_of_universities_per_capita, y = value, color = source)) +
 ...
 scale_color_manual(name = "Data source", 
  ...
  ylab("Proportion of sample") +
  xlab("Number of universities per capita") +
  theme(axis.text = element_text(size = 20),
        axis.title = element_text(size = 24),
        panel.background = element_rect(fill = "white"),
        axis.line = element_line(color = "black"),
...
researcher <- ggplot(combined_data1, aes(x = number_of_psychology_researchers_per_capita, y = value, color = source)) +
 ...
 scale_color_manual(name = "Data source", 
  ...
  ylab("Proportion of sample") +
  xlab("Number of psychology researchers per capita") +
  theme(axis.text = element_text(size = 20),
        axis.title = element_text(size = 24),
        panel.background = element_rect(fill = "white"),
        axis.line = element_line(color = "black"),
...
Network1 <- ggplot(combined_data5, aes(x = Network_2021, y = value, color = source)) +
 ...
 scale_color_manual(name = "Data source", 
  ...
  ylab("Proportion of sample") +
  xlab("Internet penetration rate") +
  theme(axis.text = element_text(size = 20),
        axis.title = element_text(size = 24),
        panel.background = element_rect(fill = "white"),
        axis.line = element_line(color = "black"),
...
Relation between the proportion of each state’s/province’s sample in big team science (using Ruggeri et al. (2022) as an example) and Urbanization/Internet penetration rate/English Proficiency in Kenya/Nigeria, China, and United States/Netherlands. 
...
# State/Province level drawing

## Urbanization drawing by state/province in Kenya, China, and the United States
...
## Internet penetration rate drawing by state/province in Nigeria, China, and the United States
...
## English Proficiency drawing by state/province in Nigeria, China, and Netherlands
