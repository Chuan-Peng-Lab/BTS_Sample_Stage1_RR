...
ggplot2::ggsave("Figure 2.pdf", width = 9, height = 13.5)
```
...
ggplot2::ggsave("Figure 6.pdf", width = 9, height = 13.5)
```
...
age_simulation$Site <- "traditional studies"
age_simulation
```
...
scale_color_manual(name="Sample source", values = c("BTS" ="#E41A1C", "traditional studies" =  "#377EB8")) +
  ...
  ylim(c(-39, 39))  
fig3c
ggsave("Figure 3.pdf", fig3c, device = "pdf", width = 12, height = 9)
```
...
ggsave(filename = "Figure 4.pdf", plot = combined_plot_people, device = "pdf", width = 24, height = 14)
```
...
```{r}
alpha_bts <- vegan::diversity(author_bts_clean, index = "simpson")
alpha_ps2014 <- vegan::diversity(author_ps2014_clean, index = "simpson")
alpha_wpd <- vegan::diversity(author_BTS$wpd, index = "simpson")

alpha_ps2014_leading <- vegan::diversity(leading_author_ps2014_clean, index = "simpson")
alpha_bts_leading <- vegan::diversity(leading_author_bts_clean, index = "simpson")

alpha_ps2014
alpha_bts
alpha_wpd

alpha_ps2014_leading
alpha_bts_leading
```
...
t_value1<- t_value(alpha_bts,alpha_ps2014,Var_bts,Var_ps2014)
t_value2<- t_value(alpha_wpd,alpha_bts,Var_wpd,Var_bts)
t_value3<- t_value(alpha_bts_leading,alpha_ps2014_leading,Var_bts_leading,Var_ps2014_leading)
t_value4<- t_value(alpha_wpd,alpha_bts_leading,Var_wpd,Var_bts_leading)
...
## Output Bayesian factors for diversity t-test
...
new_author_data1 <- new_author_data %>%
  filter(Country_groups != "other")



new_author_data1 <- new_author_data1 %>%
...
# 定义观测值
observed1 <- new_author_data1$percentage_author_ps2014

# 定义期望值 (例如均匀分布)
expected1 <- new_author_data1$percentage_author_bts
...
observed2 <- new_author_data1$percentage_author_bts
expected2 <- new_author_data1$percentage_wpd
...
observed3 <- new_author_data1$percentage_leading_author_ps2014
expected3 <- new_author_data1$percentage_leading_author_bts
...
observed4 <- new_author_data1$percentage_leading_author_bts
expected4 <- new_author_data1$percentage_wpd
...
BF1 <- BayesMultiNomial(
      dataset = new_author_data1,
      factor = "Country_groups",
      observed = "percentage_author_ps2014",
      expected = "percentage_author_bts")
BF1


BF2 <- BayesMultiNomial(
      dataset = new_author_data1,
      factor = "Country_groups",
      observed = "percentage_author_bts",
      expected = "percentage_wpd")
BF2
BF3 <- BayesMultiNomial(
      dataset = new_author_data1,
      factor = "Country_groups",
      observed = "percentage_leading_author_ps2014",
      expected = "percentage_leading_author_bts")
BF3

BF4 <- BayesMultiNomial(
      dataset = new_author_data1,
      factor = "Country_groups",
      observed = "percentage_leading_author_bts",
      expected = "percentage_wpd")
BF4
```
...
BF3 <- BayesMultiNomial(
      dataset = new_author_data,
      factor = "Country_groups",
      observed = "leading_author_ps2014",
      expected = "leading_author_bts")
BF3

BF4 <- BayesMultiNomial(
      dataset = new_author_data,
      factor = "Country_groups",
      observed = "leading_author_bts",
      expected = "wpd")
BF4
```

```{r cars}
df1 <- c(10, 20, 30, 40, 50)
df2 <- c(200, 200, 200, 200, 200)
df3 <- c(10, 20, 30, 40, 50)

# 使用$操作符直接赋值
new_author_data$df1 <- df1
new_author_data$df2 <- df2
new_author_data$df3 <- df3
new_author_data
BF1 <- BayesMultiNomial(
      dataset = new_author_data,
      factor = "Country_groups",
